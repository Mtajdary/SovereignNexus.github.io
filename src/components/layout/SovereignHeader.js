...
export default function SovereignHeader({ currentModule, onModuleChange }) {
  const navItems = [
    ...
    { id: 'engineering-lab', label: 'ابزارها و آزمایشگاه' },
    { id: 'projects-hub', label: 'پروژه‌ها و دانش فنی' },
    { id: 'profile-resume', label: 'درباره من و رزومه' },
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#fafafa]/90 backdrop-blur-md border-b border-gray-200 px-4 sm:px-8 py-3">
      <div className="max-w-4xl mx-auto flex items-center justify-between gap-3" dir="rtl">
        ...
        <nav className="flex items-center gap-1 bg-gray-200/80 p-1 rounded-full overflow-x-auto no-scrollbar shrink-0">
          {navItems.map((item) => {
