...
import { Sparkles, HardHat, Cpu, Eye, ArrowLeft } from 'lucide-react';

export default function Dashboard({ onNavigate }) {
  return (
    <div className="max-w-4xl mx-auto px-4 pt-10 pb-20 flex flex-col items-center text-center">
      ...
      {/* دکمه‌های کپسولی متصل به عملکرد واقعی */}
      <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-16 w-full max-w-xs sm:max-w-none">
        <motion.button 
          ...
          className="w-full sm:w-auto px-8 py-3 rounded-full bg-[#22c55e] hover:bg-[#16a34a] text-white font-bold text-sm shadow-[0_4px_16px_rgba(34,197,94,0.35)] transition-all flex items-center justify-center gap-2"
        >
          <span>ورود به خزانه پروتکل‌ها</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 w-full text-right" dir="rtl">
        <div 
          onClick={() => onNavigate && onNavigate('profile')}
          className="p-6 rounded-3xl bg-white border border-gray-200/90 shadow-sm space-y-3 cursor-pointer hover:border-emerald-400 transition-colors"
        >
